...
function ChatRow({id}: Props) {
    ...
    const [messages ] = useCollection(query(
        collection(db, 'usres', session?.user?.email!, 'chats', id, 'messages'),
        orderBy('createdAt', 'asc')
    ))

  return ( <Link href={`/chat/${id}`} className={`chatRow justify-center`} >
    <ChatBubbleLeftEllipsisIcon className="w-5 h-5  hover:text-red-500/70" />
    <p className="flex-1 hidden md:inline-flex truncate">New Chat</p>
    <TrashIcon className="w-5 h-5 text-gray-600 hover:text-red-500/70"/>
    <div>
